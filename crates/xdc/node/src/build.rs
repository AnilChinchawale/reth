//! XDC Block Assembler
//!
//! This module provides block assembly for XDC Network
...
/// XDC block assembler
///
/// Assembles blocks for XDC Network with XDPoS-specific logic
#[derive(Debug, Clone)]
pub struct XdcBlockAssembler<C = reth_chainspec::ChainSpec> {
    /// Chain specification
    pub chain_spec: Arc<C>,
}

impl<C> XdcBlockAssembler<C> {
    /// Create a new XDC block assembler
    pub const fn new(chain_spec: Arc<C>) -> Self {
        Self { chain_spec }
    }
}

impl<F, C> BlockAssembler<F> for XdcBlockAssembler<C>
where
